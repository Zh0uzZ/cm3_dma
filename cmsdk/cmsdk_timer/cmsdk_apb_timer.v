...
//-----------------------------------------------------------------------------
// Derrived from CMSDK timer, with added PRIV support.
// Used CMSDK release: BP210-BU-00000-r1p0-01rel0
//    Revision: 233272
//    Date: 2013-01-10 15:38:18 +0000 (Thu, 10 Jan 2013)
//    Release Information : CM3DesignStart-r0p0-02rel0
//-----------------------------------------------------------------------------
// Programmer's model
// 0x00 RW    CTRL[3:0]
...
module cmsdk_apb_timer (
  ...
  input  wire [2:0]  PPROT,    // Protection type. Only PPROT[0] is used 0 indicates NORMAL access and 1 indicates PRIVILEGED
  ...
  input  wire        PRIVMODEN,// Control whether time regs can be written in NON-PRIV mode or not
  input  wire        EXTIN,    // Extenal input
...
wire          ext_in_enable; // enable control for external intput
...
assign  write_enable = PSEL & (~PENABLE) & PWRITE & (PPROT[0] | ~PRIVMODEN); // assert for 1st cycle of write transfer
...
// and improve timing
  ...
  // lower 8 bits -registered. Current value reigster mux not done here
...
always @(PADDR or reg_ctrl or reg_reload_val or reg_timer_int or
  ...
  if (write_enable04)
    ...
    if (reg_curr_val == {32{1'b0}})
      ...
      nxt_curr_val = reg_curr_val - 32'h00000001; // Decrement
    end
  else
